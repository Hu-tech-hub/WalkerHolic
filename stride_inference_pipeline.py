#!/usr/bin/env python3
"""
Stride Inference Pipeline - 간소화된 추론 전용 버전
라벨링된 IMU 데이터에서 stride cycle을 추출하고 학습된 TCN 모델로 보폭 예측

Usage:
    python stride_inference_pipeline.py --labels support_labels.csv --walking walking_data.csv
    python stride_inference_pipeline.py --input_dir predict_support_data --walking_dir filtering_walking_data
            ...
            if norm_file.exists():
                ...
                if 'sequence_mean' in norm_data.files:
                    ...
                    self.auxiliary_stats = {
                        'height_mean': norm_data.get('height_mean', 170.0),
                        'height_std': norm_data.get('height_std', 10.0),
                        'stride_time_mean': norm_data.get('stride_time_mean', 1.0),
                        'stride_time_std': norm_data.get('stride_time_std', 0.3)
                    }
                else:
                    self.normalization_stats = {
                        'mean': norm_data['mean'],
                ...
                logger.warning("정규화 통계 파일을 찾을 수 없습니다. 기본값 사용")
                self.normalization_stats = {
                    'mean': np.zeros(6),
                    'std': np.ones(6)
                }
...
def main():
    ...
    parser.add_argument('--labels', help='Support labels CSV 파일')
    parser.add_argument('--walking', help='Walking data CSV 파일')
    ...
    if not args.labels or not args.walking:
        logger.error("--labels와 --walking 인자가 필요합니다.")
        sys.exit(1)
